...
const Categories: React.FC = () => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = allCategories.map((n: any) => n.name);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - allCategories.length) : 0;

  const navigate = useNavigate();

  const onEditClick = (event: React.MouseEvent<unknown>, row: any) => {
    window.localStorage.setItem("selectedCategory", JSON.stringify(row));
    navigate("update");
  };
        ...
        <TableContainer>
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={allCategories.length}
              user={user}
            />
            <TableBody>
              {stableSort(allCategories, getComparator(order, orderBy))
